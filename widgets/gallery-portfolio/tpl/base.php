...
//	wp_enqueue_script('templaza-element-fancybox');
    $categories = array();
    $html       = '';
    $album_id   = isset($instance['element_id'])?$instance['element_id']:uniqid();
...
//            apply_filters('templaza_gallery_post_thumbnail_height', 440), get_the_title());
        ...
        $html   .= '<a href="'.$src[0].'" data-elementor-open-lightbox="yes" data-elementor-lightbox-slideshow="'
            .$album_id.'" data-elementor-lightbox-title="'.esc_attr(get_the_title()).'" class="uk-transition-toggle" data-caption="'
            .get_the_title().'">';
    ...
    <div data-uk-filter="target: .js-filter">
        ...
        <ul class="js-filter<?php echo $columns ? ' uk-grid-small uk-child-width-1-2@s uk-child-width-1-' . $columns . '@m' : ''; ?>"
            data-uk-grid="masonry: true; parallax: 80">
            <?php echo $html; ?>
        </ul>
    </div>
